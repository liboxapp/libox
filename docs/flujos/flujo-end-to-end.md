...
Esquema maestro del **flujo de procesos** de Libox: el camino completo de una rifa desde que un organizador se registra hasta que el dinero se liquida y el sorteo queda auditado. Es la "vista de pájaro" que conecta los [8 dominios del sistema](../archive/plans/libox-plan.md#3-dominios-del-sistema-bounded-contexts) y respeta las decisiones cerradas (custodia [Z.1](../archive/decisions/Z1-custodia-del-dinero.md), PSP [Z.2](../archive/decisions/Z2-eleccion-psp.md), organizador [Z.3](../archive/decisions/Z3-tipo-de-organizador.md), motor de sorteo [Z.4](../archive/decisions/Z4-tipos-de-sorteo.md)).
...
El sorteo es la entidad central; su ciclo de vida gobierna qué acciones son válidas en cada momento (state machine, ver glosario). La rama `FAILED` es el acople invisible con pagos: si no se cumple la condición de disparo al deadline, **se reembolsa a todos** (ver [Z.4](../archive/decisions/Z4-tipos-de-sorteo.md)).
...
**A · Onboarding (dominio 1).** Solo organizadores con **RUC activo** (natural o jurídica, [Z.3](../archive/decisions/Z3-tipo-de-organizador.md)). El KYC valida el RUC contra el padrón SUNAT; el organizador queda como *merchant of record* (emite el comprobante, no Libox).
...
**C · Compra (dominios 3 y 4).** El punto más crítico por concurrencia y dinero. Pago con **Mercado Pago, split en la fuente** ([Z.2](../archive/decisions/Z2-eleccion-psp.md)): el dinero del participante nunca pasa por Libox (Modelo C, [Z.1](../archive/decisions/Z1-custodia-del-dinero.md)). `Idempotency-Key` + webhook firmado evitan dobles cobros y dobles asignaciones. Recién con el pago consolidado se asigna número de boleto y se emite el comprobante vía PSE. El ledger doble entrada corre desde el día 1 aunque la custodia sea conceptual.
...
- Validar con el abogado la evidencia digital del sorteo y el umbral de IR ([compliance-peru.md](../archive/compliance-peru.md)).
