...
- **Shared Claude config (versioned).** `.claude/settings.json` (team `SessionStart` hook + `enabledPlugins`/`extraKnownMarketplaces`) and `.claude/skills/` (67 shared skills) are **committed** so every teammate gets the same environment on clone; plugins install themselves from their public marketplaces. `.claude/settings.local.json` stays **personal and gitignored** — never commit it. `.claude/skills/**` is excluded from markdownlint (third-party content, not wiki prose).
